...
public class TestingOutLombokDependency extends HR_ORDS_TestBase {
    ...
    @DisplayName("GET /employees and filter the result with JsonPath groovy")
    @Test
    public void testFilterResultWithGroovyPractice(){

        JsonPath jp = get("/employees").jsonPath();

        List<Employees> emp=jp.getList("items.findAll{it.employee_id>150}.",Employees.class);
        ...
        List<String> firstName=jp.getList("items.first_name");
        System.out.println("firstName = " + firstName);
        
        List<String> firstNamefilt=jp.getList("items.findAll{it.department_id>50}.first_name");
        System.out.println("firstNamefilt = " + firstNamefilt);
...
//        System.out.println("allDepIdsFiltered = " + allDepIdsFiltered);
        ...
        List<Integer> allEmp=jp.getList("items.department_id");
        System.out.println("allEmp = " + allEmp);
        
        
        List<Integer> filt=jp.getList("items.findAll{it.department_id>70}.department_id");
...
//        System.out.println("deps70to100 = " + deps70to100);
        ...
        List<String>Empl=jp.getList("items.findAll{it.salary>=2500&&it.salary<=10000}.first_name");
        System.out.println("Empl = " + Empl);
        
        int maxSal=jp.getInt("items.salary.max()");
        System.out.println("maxSal = " + maxSal);

        String empH= jp.getString("items.find{it.salary="+maxSal+"}.first_name");
        System.out.println("empH = " + empH);

        String empH1= jp.getString("items.find{it.salary="+maxSal+"}.phone_number");
        System.out.println("empH = " + empH1);


        int minSalfiacc=jp.getInt("items.FI_ACCOUNT.find{it.salary.max()}");
        System.out.println("maxSal = " + minSalfiacc);
